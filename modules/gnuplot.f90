...
contains
	...
	subroutine scatter_plot2D(x_values,y_values,aspect_ratio_in,filename_in,xlabel_in,ylabel_in)
		!	--------------------------
		!	This subroutine uses gnuplot to plot
		...
		character(*),intent(in),optional		::	xlabel_in,ylabel_in						
		...
		write(script_file,*)"set output '" // trim(plot_dir) // "/" // trim(filename) // "'"
		if (present(xlabel_in)) then
			write(script_file,*)"set xlabel '" // trim(xlabel_in) // "'"
		endif
		if (present(ylabel_in)) then
			write(script_file,*)"set ylabel '" // trim(ylabel_in) // "'"
		endif
